...
RegisterNUICallback('cookingComplete', function(data, cb)
    DebugPrint('Cocción completada: ' .. json.encode(data))
...
RegisterNUICallback('startCooking', function(data, cb)
    DebugPrint('Iniciando cocción: ' .. json.encode(data))
    ...
    while true do
        ...
        for i, stationCoords in ipairs(Config.CookingStations) do
            ...
            if distance <= 3.0 then
                ...
                DrawText3D(stationCoords.x, stationCoords.y, stationCoords.z + 1.0, '[E] Cocinar')
